...
public class DwmOrderWideApp extends BaseAppV2 {
    ...
    public static void main(String[] args) {
        new DwmOrderWideApp().init(3004,1,"DwmOrderWideApp","DwmOrderWideApp",
        ...
        factJoinedStream
                .map(new RichMapFunction<OrderWide, OrderWide>() {
                    ...
                    public OrderWide map(OrderWide orderWide) throws Exception {
                        ...
                        //1.user_info
                        JSONObject userInfo = DimUtil.readFromPhoenix(connection, DIM_USER_INFO, orderWide.getUser_id());
                        orderWide.setUser_gender(userInfo.getString("GENDER"));
                        orderWide.calcUserAge(userInfo.getString("BIRTHDAY"));
                        
                        //2.province
                        JSONObject provinceInfo = DimUtil.readFromPhoenix(connection, DIM_BASE_PROVINCE, orderWide.getProvince_id());
                        ...
                        //3.sku_info
                        JSONObject skuInfo = DimUtil.readFromPhoenix(connection, DIM_SKU_INFO, orderWide.getSku_id());
                        ...
                        // 4. 读取tm信息
                        JSONObject tmInfo = DimUtil.readFromPhoenix(connection, DIM_BASE_TRADEMARK, orderWide.getTm_id());
                        orderWide.setTm_name(tmInfo.getString("TM_NAME"));
    
                        // 5. 读取spu信息
                        JSONObject spuInfo = DimUtil.readFromPhoenix(connection, DIM_SPU_INFO, orderWide.getSpu_id());
                        orderWide.setSpu_name(spuInfo.getString("SPU_NAME"));
    
                        // 6. 读取c3信息
                        JSONObject c3Info = DimUtil.readFromPhoenix(connection, DIM_BASE_CATEGORY3, orderWide.getCategory3_id());
